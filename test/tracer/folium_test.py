...
def test_add_trace_start_to_map(folium_map, stops_df):
    ...
    assert any(
        isinstance(child, folium.CircleMarker)
        for child in folium_map._children.values()
    )


def test_add_trace_footpath_to_map(folium_map, stops_df):
    trace_start = TraceStart("stop1", 0)
    ...
    assert any(
        isinstance(child, folium.PolyLine) for child in folium_map._children.values()
    )
    assert any(
        isinstance(child, folium.CircleMarker)
        for child in folium_map._children.values()
...
def test_add_trace_trip_to_map(folium_map, stops_df):
    trace_start = TraceStart("stop1", 0)
...
def test_add_multiple_tracer_types_to_map(folium_map, stops_df):
    trace_start = TraceStart("stop1", 0)
    trace_footpath = TraceFootpath("stop1", "stop2", 30)
    trace_trip = TraceTrip("stop1", 10, "stop2", 20, "trip1")
...
def test_add_unknown_tracer_type(folium_map, stops_df):
