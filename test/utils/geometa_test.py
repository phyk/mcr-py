...
def test_hash_boundary(geo_meta):
...
def test_get_bounding_box(geo_meta):
    bbox = geo_meta.get_bounding_box()
    assert len(bbox) == 4  # (minx, miny, maxx, maxy)


def test_get_bounding_box_as_coord_list(geo_meta):
    coord_list = geo_meta.get_bounding_box_as_coord_list()
    assert len(coord_list) == 4  # Should return 4 coordinates


def test_load_and_save(geo_meta, tmp_path):
    # Test saving
    save_path = tmp_path / "geometadata.json"
    geo_meta.save(str(save_path))

    # Test loading
    loaded_geo_meta = GeoMeta.load(str(save_path))
...
def test_get_center_lat_lon(geo_meta):
...
def test_add_to_folium_map(geo_meta):
    import folium
